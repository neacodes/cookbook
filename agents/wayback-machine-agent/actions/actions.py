...
def get_wayback_changes(url: str, days: int) -> str:
    ...
    """
    user_agent = "Sema4.ai Desktop"
    cdx_api = WaybackMachineCDXServerAPI(url, user_agent)
    end_date = datetime.now()
    start_date = end_date - timedelta(days)
    start_archive = cdx_api.near(year=start_date.year, month=start_date.month, day=start_date.day, hour=start_date.hour, minute=start_date.minute)
    end_archive = cdx_api.near(year=end_date.year, month=end_date.month, day=end_date.day, hour=end_date.hour, minute=end_date.minute)
